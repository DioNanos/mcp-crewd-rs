#!/usr/bin/env node
// mock-codex-appserver.mjs — fixture for the CodexAdapter contract tests
// (crewd Phase 2 Task 11). Speaks JSON-RPC 2.0 NDJSON over stdio WITHOUT the
// `codex` binary installed. Answers initialize / thread/start / turn/start /
// turn/interrupt / thread/resume with deterministic fixtures; emits the
// turn/completed notification after turn/start.
//
//   CODEX_MOCK_LOG=<path>   if set, appends one JSON line {method,params} per
//                           request received (so the test can verify the
//                           serialized fields).
//   CODEX_MOCK_POLICY=untrusted  makes thread/start and thread/resume reply
//                           with approvalPolicy:"untrusted" for the fail-clear
//                           test (the adapter must reject with E_POLICY_DENIED).
