...
**Status:** v0.1; same-UID L0 posture **ratified by the maintainer** (§4.0); Phase 0 gate G1
...
the `crewd` code in Phase 1 must satisfy; §7 (hardening checklist) becomes acceptance
criteria.
**Binding inputs (honored, deviations only with written rationale in §10):**
an external design/audit review, findings F2/F6/F7/F8 (this
...
own runtime env (`~/.claude`, `~/.codex`, third-party endpoint bearers), not in `crewd`. The bus holds
...
|---|---|---|
| Human (operator) | Sole permission approver; interacts via a client over the documented API | Trusted |
...
> written to be ratified (or rejected) by the maintainer before Phase 1 code. It deliberately does
...
> **Ratified by the maintainer** (external security-posture review,
> ratifiable-with-modifications). The block below is reproduced **verbatim** from
> the external review's prescribed text for this section. It is normative; its
> sentences appear here textually, not paraphrased.
...
### 4.3 v0 decision (ratified by the maintainer)
...
If the maintainer does **not** ratify this, the project MUST adopt one of the §4.5 upgrade paths
before Phase 1, or narrow v0 to a single cell + human (no peer cells).
...
|---|---|---|---|
| **v0 (same-UID)** | nothing between cells; other UIDs + network excluded | lowest — what Phase 1 ships | single trusted operator, cooperative cells |
...
Acceptance criteria for the `crewd` code in Phase 1. Each item is normative (MUST/SHOULD)
...
These references keep the two documents in lockstep for Phase 1.
...
- **Retired upstream claim.** The design doc (§4.2) and `README.md` charter originally
  described provenance with strong "cannot-be-forged" wording (the original charter term).
...
- **Gate G1 ratifications (2026-07-03, applied in this revision).** **D1** — same-UID L0
  posture ratified by the maintainer; the §4.0 block is verbatim text and **ratified**, no longer
...
original charter wording for forgery-proof, returns **zero hits** across this
