...
impl Store {
    ...
    pub fn transition(&self, message_id: &str, recipient: &str, to: DeliveryState) -> Result<()> {
...
mod tests {
    ...
    fn transition_table_is_exact() {
        ...
        assert!(!crate::state::transition_allowed(S::Queued, S::Delivered)); // no skipping states
    ...
    fn seq_is_per_pair_monotonic() {
        ...
        assert_eq!(s.next_seq("a", "c").unwrap(), 1); // different pair, its own counter
    }
    #[test]
    ...
    fn retry_budget_exhaustion_fails() {
        ...
        for _ in 0..10 {
            exhausted = s
                .record_attempt_failure(&e.message_id, "bob", 10, 1, 60)
                .unwrap();
        }
        assert!(exhausted);
    ...
    fn dedupe_binds_key_to_content() {
        let s = Store::open_in_memory().unwrap();
        s.dedupe_insert("alice", "key1", "m1", None, "hashA")
            .unwrap();
        let hit = s.dedupe_lookup("alice", "key1").unwrap().unwrap();
        assert_eq!(hit.message_id, "m1");
        assert!(
            s.dedupe_lookup("bob", "key1").unwrap().is_none(),
            "scoped per sender"
        );
    }
}
