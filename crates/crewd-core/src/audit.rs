...
/// Fase 1 kinds followed by the 12 Fase 2 cell-fabric kinds.
pub const AUDIT_KINDS: [&str; 29] = [
    // --- 17 Fase 1 kinds (SPEC §12.2) ---
    "enqueued",
    "delivered",
    "acked",
    "delivery_failed",
...
impl AuditChain {
    ...
    pub fn open(path: &Path) -> io::Result<Self> {
        let head_hash = if path.exists() {
            ...
            match raw.lines().filter(|l| !l.trim().is_empty()).last() {
                Some(line) => {
    ...
    pub fn verify(path: &Path) -> VerifyResult {
        ...
        let raw = match std::fs::read_to_string(path) {
            Ok(s) => s,
            Err(_) => return VerifyResult::Broken { event_id: String::new(), index: 0 },
        };
...
mod tests {
    ...
    fn chain_appends_and_verifies() {
        ...
        let e1 = c
            .append(AuditEventDraft::new("enqueued", Some("m1"), Some("a"), Some("b"), "ok", None))
            .unwrap();
        assert_eq!(e1.prev_hash, "0".repeat(64));
        let e2 = c
            .append(AuditEventDraft::new("delivered", Some("m1"), Some("a"), Some("b"), "ok", None))
            .unwrap();
    ...
    fn one_byte_flip_breaks_verification() {
        ...
        let p = dir.path().join("audit.jsonl");
        let mut c = AuditChain::open(&p).unwrap();
        c.append(AuditEventDraft::new("enqueued", Some("m1"), Some("a"), Some("b"), "ok", None))
            .unwrap();
        let ev2 = c
            .append(AuditEventDraft::new("delivered", Some("m1"), Some("a"), Some("b"), "ok", None))
            .unwrap();
        let mut raw = std::fs::read_to_string(&p).unwrap();
        raw = raw.replacen("\"outcome\":\"ok\"", "\"outcome\":\"OK\"", 1); // flip nel primo evento
    ...
    fn reopen_resumes_head() {
        ...
        let h1 = {
            let mut c = AuditChain::open(&p).unwrap();
            c.append(AuditEventDraft::new("enqueued", None, None, None, "ok", None))
                .unwrap()
                .hash
        };
        let mut c2 = AuditChain::open(&p).unwrap();
        let e2 = c2
            .append(AuditEventDraft::new("expired", None, None, None, "expired", None))
            .unwrap();
    ...
    /// SPEC §20.10: the 12 new Fase 2 audit event kinds are accepted by the
    /// chain; hash-linkage and end-to-end verification stay intact.
    #[test]
    fn fase2_audit_kinds_accepted() {
        let dir = tempfile::tempdir().unwrap();
        let p = dir.path().join("audit.jsonl");
        let mut c = AuditChain::open(&p).unwrap();
        let kinds = [
        ...
        for k in kinds {
            let ev = c
                .append(AuditEventDraft::new(k, Some("m1"), Some("a"), Some("b"), "ok", None))
                .unwrap();
            assert_eq!(ev.kind, k);
        ...
        match AuditChain::verify(&p) {
            ...
            _ => panic!("chain must verify with fase2 kinds"),
        }
    }
}
