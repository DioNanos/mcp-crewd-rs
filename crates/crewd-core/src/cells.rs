...
impl Store {
    ...
    pub fn cell_register(&self, def: &CellDef, audit: &mut AuditChain) -> Result<(), BusError> {
        ...
        let present = self
            .0
            .query_row("SELECT 1 FROM cells WHERE name = ?1", params![&def.name], |_| Ok(()))
            .is_ok();
        if present {
            return Err(BusError::Internal(format!("cell already exists: {}", def.name)));
        }
    ...
    pub fn cell_update(&self, def: &CellDef, audit: &mut AuditChain) -> Result<(), BusError> {
        ...
        let present = self
            ...
            .is_ok();
        if !present {
            return Err(BusError::UnknownCell(format!("cell not found: {}", def.name)));
        }
        let detail = serde_json::json!({
            "engine": def.engine.as_str(),
            "cwd": def.cwd,
...
pub fn resolve_spawn_target(
    ...
    match named {
        ...
        None => {
            let engine = req_engine
                .ok_or_else(|| BusError::PolicyDenied("ephemeral spawn requires an engine".into()))?;
...
mod tests {
    ...
    fn registry_roundtrip_and_immutability() {
        ...
        let def = sample_def("glm-worker-a");
        s.cell_register(&def, &mut audit).unwrap();
        assert!(s.cell_register(&def, &mut audit).is_err()); // duplicate register = error
        let got = s.cell_get("glm-worker-a").unwrap().unwrap();
        assert_eq!(got.model.as_deref(), Some("glm-5.2"));
        // named + override → E_POLICY_DENIED
        let r = resolve_spawn_target(Some(got), None, Some("gpt-5.5".into()), None, None);
        assert!(matches!(r, Err(e) if e.code() == "E_POLICY_DENIED"));
        // ephemeral: engine mandatory
        let e =
            resolve_spawn_target(None, Some(EngineKind::Codex), None, None, Some("/tmp/x".into()))
                .unwrap();
        assert!(matches!(e, SpawnTarget::Ephemeral { engine: EngineKind::Codex, .. }));
    }
    ...
    fn cell_update_existing_succeeds() {
        ...
        s.cell_register(&def, &mut audit).unwrap();
        def.model = Some("glm-5.2-next".into());
        s.cell_update(&def, &mut audit).unwrap();
        let got = s.cell_get("c1").unwrap().unwrap();
        assert_eq!(got.model.as_deref(), Some("glm-5.2-next"));
    }
    ...
    fn cell_list_defs_orders_by_name() {
        ...
        let names: Vec<_> = s.cell_list_defs().unwrap().into_iter().map(|d| d.name).collect();
    ...
    fn engine_kind_roundtrip() {
        for k in [EngineKind::Codex, EngineKind::Claude, EngineKind::Pi, EngineKind::Fake] {
            assert_eq!(EngineKind::from_db_str(k.as_str()), Some(k));
        }
        assert_eq!(EngineKind::from_db_str("nope"), None);
    }

    // --- AUDIT1 R1: audit-before-mutation failure + validator coverage ---
    ...
    fn cell_update_audit_failure_writes_no_mutation() {
        ...
        assert_eq!(after.model, original.model, "update must not land on audit failure");
    }
}
