...
"#;
    ...
    fn parse_and_check_capability() {
        ...
        assert_eq!(
            acl.check("ghost", Capability::Send).unwrap_err().code(),
            "E_ACL_DENIED"
        );
    }
    #[test]
    ...
    fn protected_reach_requires_per_target_grant() {
        let acl = Acl::parse(ACL).unwrap();
        assert!(acl
            .check_reach("dev-senior", "coordinator", ReachVia::Ask)
            .is_ok());
        assert_eq!(
        ...
        assert!(
            acl.check_reach("codex-audit", "dev-senior", ReachVia::Send)
                .is_ok(),
            "non-protected: no gate"
        );
    }
    #[test]
    fn broadcast_fanout_omits_ungranted_protected() {
        ...
        assert!(
            ...
            "G1-01: never via the broadcast capability alone"
        );
        assert_eq!(denied, vec!["coordinator".to_string()]);
        let (inc2, den2) = acl.recipients_for_broadcast("dev-senior");
        assert!(
            inc2.contains(&"coordinator".to_string()),
            "included with the grant"
        );
    ...
    fn atomic_reload_keeps_previous_on_invalid() {
        ...
        assert!(
            holder
                .current()
                .check("dev-senior", Capability::Send)
                .is_ok(),
            "previous ACL still active"
        );
    }
}
