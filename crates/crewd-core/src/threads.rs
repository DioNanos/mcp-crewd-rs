...
mod tests {
    ...
    fn thread_active_for_cell_only_spawning_running() {
        ...
        assert!(s.thread_active_for_cell("worker-a").unwrap().is_none());
        // a running thread is active
        let run = sample_thread(ThreadState::Running);
        s.thread_insert(&run).unwrap();
        let act = s.thread_active_for_cell("worker-a").unwrap().unwrap();
        ...
        let still = s.thread_active_for_cell("worker-a").unwrap().unwrap();
