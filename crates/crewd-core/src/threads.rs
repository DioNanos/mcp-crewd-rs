...
const THREAD_COLS: &str = "crewd_thread_id, cell_name, engine_kind, model, profile, engine_process_id,\
...
impl ThreadRaw {
    ...
    fn into_thread(self) -> Result<CellThread, BusError> {
        let engine_kind = EngineKind::from_db_str(&self.engine_kind)
            .ok_or_else(|| BusError::Internal(format!("unknown engine kind: {}", self.engine_kind)))?;
...
impl Store {
    ...
    pub fn thread_get(&self, id: &str) -> Result<Option<CellThread>, BusError> {
        ...
        let res = self
            .0
            .query_row(&sql, params![id], ThreadRaw::from_row);
        match res {
    ...
    pub fn thread_set_engine_ids(
        ...
        let n = self
            ...
            .execute(
                ...
                params![id, process_id, thread_id, turn_id, session_id, now_rfc3339()],
            )
            .map_err(|e| BusError::Internal(e.to_string()))?;
        if n == 0 {
            return Err(BusError::Internal(format!("thread not found: {id}")));
    ...
    /// Boot recovery (AUDIT2 B1 / chaos §20 test b): list every thread whose
    ...
    pub fn threads_in_states(&self, states: &[ThreadState]) -> Result<Vec<CellThread>, BusError> {
        ...
        let mut stmt = self.0.prepare(&sql).map_err(|e| BusError::Internal(e.to_string()))?;
...
mod tests {
    ...
    fn thread_store_roundtrip_ids_separated() {
        ...
        s.thread_transition(&t.crewd_thread_id, ThreadState::Running).unwrap();
        assert!(s.thread_transition(&t.crewd_thread_id, ThreadState::Spawning).is_err());
