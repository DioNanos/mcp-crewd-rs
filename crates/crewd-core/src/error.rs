...
pub enum BusError {
    ...
    // --- Fase 2 (SPEC §20.10): 7 new error codes ---
    #[error("E_CELL_BUSY: {0}")]
    CellBusy(String),
...
mod tests {
    ...
    /// SPEC §20.10: the 7 new Fase 2 error codes, verbatim. Each variant
    /// carries a human-readable message, like the Fase 1 variants.
    #[test]
    fn fase2_error_codes_exact() {
        for (e, s) in [
