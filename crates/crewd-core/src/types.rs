...
mod tests {
    ...
    fn error_codes_are_exact_set() {
        ...
        let codes: Vec<&str> = vec![
            E::AclDenied(String::new()).code(),
            E::AuthRejected(String::new()).code(),
            E::UnknownCell(String::new()).code(),
            E::TtlExpired(String::new()).code(),
            E::WouldDeadlock(String::new()).code(),
            E::Quota(String::new()).code(),
            E::BodyTooLarge(String::new()).code(),
            E::Dup(String::new()).code(),
            E::AskNotFound(String::new()).code(),
            E::ReplyExists(String::new()).code(),
            E::UnsupportedSpecVersion(String::new()).code(),
            E::Internal(String::new()).code(),
        ];
        assert_eq!(
            codes,
            vec![
    ...
    fn kind_and_msg_type_serde_names() {
        assert_eq!(
            serde_json::to_string(&Kind::Broadcast).unwrap(),
            "\"broadcast\""
        );
        assert_eq!(
            serde_json::to_string(&MsgType::AdminRequest).unwrap(),
            "\"admin_request\""
        );
    }
    #[test]
    ...
    fn reply_requires_reply_to() {
        ...
        e.reply_to = None;
        assert_eq!(
            e.validate_invariants().unwrap_err().code(),
            "E_ASK_NOT_FOUND"
        );
    }
    #[test]
    ...
    fn uuidv7_is_lowercase_hyphenated() {
        ...
        assert_eq!(
            u.split('-').map(|s| s.len()).collect::<Vec<_>>(),
            vec![8, 4, 4, 4, 12]
        );
    }
    #[test]
