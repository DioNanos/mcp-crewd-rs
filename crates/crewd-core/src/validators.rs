...
/// A client MUST NOT supply a name with this prefix (AUDIT2 B3): only the
...
/// AUDIT2 B3: the spawn primitive accepts either a valid **named** cell name
...
mod tests {
    ...
    fn ephemeral_name_accepted_only_via_dedicated_validator() {
        ...
        // …but is rejected by the strict named-cell validator (AUDIT2 B3).
        assert_eq!(
            validate_cell_name("~ephemeral-0190a1b2").unwrap_err().code(),
            "E_POLICY_DENIED"
        );
        // A malformed ephemeral suffix is rejected even by the lenient path.
        for bad in ["~ephemeral-", "~ephemeral-UP", "~ephemeral-a/b", "~ephemeral- x"] {
            assert_eq!(
                validate_cell_name_allowing_ephemeral(bad).unwrap_err().code(),
                "E_POLICY_DENIED",
                "expected rejection of {bad:?}"
            );
        }
    ...
    fn idempotency_key_bounds() {
        ...
        assert_eq!(
            validate_idempotency_key(&"k".repeat(129)).unwrap_err().code(),
            "E_POLICY_DENIED"
        );
    }
}
