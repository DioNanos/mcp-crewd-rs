...
impl Store {
    ...
    pub fn jobs_for_thread(&self, thread_id: &str) -> Result<Vec<CellJob>, BusError> {
        let sql = format!("SELECT {JOB_COLS} FROM cell_jobs WHERE crewd_thread_id = ?1 ORDER BY enqueue_seq ASC");
...
mod tests {
    ...
    fn fifo_deterministic_on_same_second_uses_enqueue_seq() {
        // AUDIT1 R1: created_at is second-granular; two jobs in the same second
