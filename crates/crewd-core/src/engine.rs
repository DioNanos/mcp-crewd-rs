...
impl Store {
    ...
    pub fn journal_tail(&self, thread_id: &str, n: usize) -> Result<Vec<String>, BusError> {
        ...
        Ok(out
            .into_iter()
            .map(|(seq, line)| format!("{seq:04} {line}"))
            .collect())
    }
}
...
mod tests {
    ...
    fn journal_seq_monotonic_and_tail_prefix() {
        ...
        assert_eq!(
            tail,
            vec!["0002 line-b".to_string(), "0003 line-c".to_string()]
        );
    ...
    fn engine_event_serde_roundtrip_all_tags() {
        ...
        assert!(serde_json::to_string(&EngineEvent::Accepted {
            engine_turn_id: "x".into()
        })
        .unwrap()
        .contains("\"ev\":\"accepted\""));
        assert!(
            serde_json::to_string(&EngineEvent::Note { text: "n".into() })
                .unwrap()
                .contains("\"ev\":\"note\"")
        );
        assert!(serde_json::to_string(&EngineEvent::Final {
            final_answer: "f".into()
        })
        .unwrap()
        .contains("\"ev\":\"final\""));
        assert!(
            serde_json::to_string(&EngineEvent::Failed { error: "e".into() })
                .unwrap()
                .contains("\"ev\":\"failed\"")
        );
    }
    ...
    fn journal_concurrent_file_backed_no_gaps() {
        ...
        assert_eq!(
            all, expected,
            "journal seq must be gap-free + unique under concurrency"
        );
    }
