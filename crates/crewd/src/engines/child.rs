...
//! adapters (AUDIT2 R2 B2). A background reader thread parses each stdout line
...
impl LineChild {
    ...
    pub fn spawn(
        ...
        let mut child = cmd
            .spawn()
            .map_err(|e| BusError::EngineDown(format!("{label} spawn: {}", safe_prefix(&e.to_string()))))?;
        let pid = child.id();
    ...
    pub fn send(&mut self, v: serde_json::Value) -> Result<(), BusError> {
        ...
        stdin
            ...
            .map_err(|e| BusError::EngineDown(format!("child write: {}", safe_prefix(&e.to_string()))))?;
        stdin
            .flush()
            .map_err(|e| BusError::EngineDown(format!("child flush: {}", safe_prefix(&e.to_string()))))?;
        Ok(())
    }
