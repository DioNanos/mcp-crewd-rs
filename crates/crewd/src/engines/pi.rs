//! engine-pi adapter (crewd Fase 2 Task 13; AUDIT2 R2 poll model). Spawns
...
//!
//! The turn is non-blocking (AUDIT2 B2): events arrive on the `LineChild`
    ...
    pub fn new(cfg: &EngineSpawnCfg) -> Result<Self, BusError> {
        ...
        let parse = |v: serde_json::Value, tx: &std::sync::mpsc::Sender<EngineEvent>| {
            ...
            if let Some(err) = v.get("error") {
                ...
                let _ = tx.send(EngineEvent::Failed { error: code.to_string() });
                return;
            }
            let Some(result) = v.get("result") else { return };
            ...
            match status {
                "accepted" => {
                    let turn_id = result
                        ...
                        .unwrap_or("")
                        .to_string();
                    let _ = tx.send(EngineEvent::Accepted { engine_turn_id: turn_id });
                }
                "progress" => {
                    let text = result.get("note").and_then(|x| x.as_str()).unwrap_or("").to_string();
                ...
                "aborted" => {
                    let _ = tx.send(EngineEvent::Failed { error: "aborted".into() });
                }
                _ => {}
            }
        };
