...
//! non-`max` profiles route the engine to an Anthropic-compatible endpoint
//! defined in `crewd.toml [profile.<name>]` (base_url/model + api_key_env read
//! from `keys_env_path`). Secrets never appear in argv/logs; error messages
//! carry a ≤8-char prefix.
...
impl ClaudeAdapter {
    ...
    /// For any non-`max` profile, `profile_def` AND `keys_env_path` MUST be set
    /// and parseable, else `E_ENGINE_DOWN` is returned (no `HOME` is read).
...
fn build_env(cfg: &EngineSpawnCfg) -> Result<HashMap<String, String>, BusError> {
    ...
    match cfg.profile.as_deref() {
        None | Some("max") => {}
        Some(name) => {
            let def = cfg.profile_def.as_ref().ok_or_else(|| {
                BusError::EngineDown(format!("unknown claude profile: {}", safe_prefix(name)))
            })?;
            let path = cfg.keys_env_path.as_ref().ok_or_else(|| {
                BusError::EngineDown(format!("profile {name:?} requires keys_env_path"))
            })?;
            let key = parse_key_file(path, &def.api_key_env)?;
            env.insert("ANTHROPIC_BASE_URL".into(), def.base_url.clone());
            env.insert("ANTHROPIC_AUTH_TOKEN".into(), key);
            env.insert("ANTHROPIC_MODEL".into(), def.model.clone());
        }
    }
    Ok(env)
}
