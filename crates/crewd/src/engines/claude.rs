//! engine-claude adapter (crewd Fase 2 Task 12-rust; AUDIT2 R2 poll model).
...
//!                   `{"ev":"error","error":"…"}`
//!
//! The turn is **non-blocking** (AUDIT2 B2): `start_turn` writes the op and
...
impl ClaudeAdapter {
    ...
    pub fn new(cfg: &EngineSpawnCfg) -> Result<Self, BusError> {
        ...
        let mut cmd = if cfg.spawn_direct {
            ...
            let shim_path = cfg.bin_override.as_deref().unwrap_or("shim/claude-shim.mjs");
            // smoke-T16: il path shim va risolto ASSOLUTO prima di current_dir
            // (relativo alla cwd del daemon, non a quella richiesta per il child).
...
impl EngineAdapter for ClaudeAdapter {
    ...
    fn interrupt(&mut self) -> Result<(), BusError> {
        // Non-blocking (AUDIT2 M5): write `abort` and return. The shim processes
...
fn build_env(cfg: &EngineSpawnCfg) -> Result<HashMap<String, String>, BusError> {
    ...
    match cfg.profile.as_deref() {
        ...
        Some(p) if p == "zai-a" || p == "zai-p" => {
            ...
            let wanted = if p == "zai-a" { "ZAI_API_KEY_A" } else { "ZAI_API_KEY_P" };
...
fn parse_key_file(path: &str, wanted: &str) -> Result<String, BusError> {
    let content = std::fs::read_to_string(path).map_err(|e| {
        BusError::EngineDown(format!("keys file unreadable: {}", safe_prefix(&e.to_string())))
    })?;
