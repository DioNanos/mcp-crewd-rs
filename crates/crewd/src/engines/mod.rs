//! Engine adapter contract + factory (crewd Phase 2 Task 8).
//!
...
//! Accepted**: `start_turn` writes the turn request and returns
...
//!
//! Resume is **typed**: `resume_thread` (codex `thread/resume`,
...
pub trait EngineAdapter: Send {
    ...
    /// Begin a turn. **Non-blocking**: writes the request and
...
pub struct EngineSpawnCfg {
    ...
    /// so it can be `kill -9`'d (Task 15).
    pub spawn_direct: bool,
    /// Path to the keys file (e.g. `~/.config/keys/ai.env`) for non-`max`
    /// profiles; `None` → such profiles fail `E_ENGINE_DOWN` (no HOME read).
    pub keys_env_path: Option<String>,
    /// Resolved profile definition (base_url/model/api_key_env) from
    /// `crewd.toml [profile.<name>]`. `None` for the built-in `max` profile
    /// (host default credentials) or when no profile is set.
    pub profile_def: Option<crate::config::ProfileDef>,
