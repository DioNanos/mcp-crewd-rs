//! engine-codex adapter (crewd Fase 2 Task 11; AUDIT2 R2 M3/M4). Spawns
...
//!
//! Turn shape (AUDIT2 B2): `start_turn` sends `turn/start` and blocks only for
...
impl CodexAdapter {
    pub fn new(cfg: &EngineSpawnCfg) -> Result<Self, BusError> {
        ...
        let mut child = cmd
            .spawn()
            .map_err(|e| BusError::EngineDown(format!("codex spawn: {}", safe_msg(&e.to_string()))))?;
    ...
    fn send(&mut self, v: Value) -> Result<(), BusError> {
        ...
        stdin
            ...
            .map_err(|e| BusError::EngineDown(format!("codex write: {}", safe_msg(&e.to_string()))))?;
        stdin
            .flush()
            .map_err(|e| BusError::EngineDown(format!("codex flush: {}", safe_msg(&e.to_string()))))?;
        Ok(())
    }
...
impl EngineAdapter for CodexAdapter {
    ...
    fn start_turn(&mut self, payload: &str) -> Result<(), BusError> {
        ...
        // asynchronously via `poll_events` (AUDIT2 B2). A turn/start `error`
        ...
        let turn_id = resp
            ...
            .to_string();
        self.pending
            .push_back(EngineEvent::Accepted { engine_turn_id: turn_id.clone() });
...
fn verify_yolo(resp: &Value) -> Result<(), BusError> {
    ...
    let st = resp.get("sandbox").and_then(|v| v.get("type")).and_then(|v| v.as_str());
...
fn safe_msg(s: &str) -> String {
    s.chars().take(8).collect::<String>().replace(['\n', '\r'], " ")
}
...
mod tests {
    ...
    /// Schema app-server v2 REALE (`ThreadItem::agentMessage`): il testo sta
    /// nel campo flat `text`, NON in `content[]` a blocchi. Regressione
    /// 2026-07-05: l'adapter cercava solo `content[]` → `final_answer` sempre
    /// vuoto sui turni codex reali (report dell'auditor perso).
    #[test]
    ...
    /// Turni reali emettono PIÙ agentMessage (note di progresso + risposta
    /// finale): il final answer è l'ULTIMO, non il concat di tutti.
    #[test]
    ...
    /// Fallback legacy: shape a blocchi `content[].{type:text,text}` ancora
    /// accettata se il campo `text` flat manca.
    #[test]
    ...
    fn verify_yolo_accepts_never_danger() {
        let resp = serde_json::json!({"approvalPolicy": "never", "sandbox": {"type": "dangerFullAccess"}});
    ...
    fn verify_yolo_if_present_ignores_absent_but_catches_downgrade() {
        ...
        assert_eq!(verify_yolo_if_present(&bad).unwrap_err().code(), "E_POLICY_DENIED");
    }
