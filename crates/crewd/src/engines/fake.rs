//! In-process `FakeEngine` (crewd Phase 2 Task 8; poll model). No child
...
impl EngineAdapter for FakeEngine {
    ...
    fn start_turn(&mut self, payload: &str) -> Result<(), BusError> {
        ...
        self.queued.push_back(EngineEvent::Accepted {
        ...
        if self.fail_next {
            self.queued.push_back(EngineEvent::Failed {
                error: "fake-failure".into(),
            });
        ...
        self.queued.push_back(EngineEvent::Final {
            final_answer: format!("done: {payload}"),
        });
        self.in_flight = false;
        Ok(())
    }
...
mod tests {
    ...
    fn happy_path_accepted_then_final_in_order() {
        ...
        assert_eq!(evs.len(), 2);
        assert!(
            matches!(&evs[0], EngineEvent::Accepted { engine_turn_id } if engine_turn_id == "fake-turn-1")
        );
        assert!(
            matches!(&evs[1], EngineEvent::Final { final_answer } if final_answer == "done: hello")
        );
    }
