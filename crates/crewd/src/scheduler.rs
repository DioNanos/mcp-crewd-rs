//! Job scheduler + per-turn timeout (crewd Fase 2 Task 9; AUDIT2 R2).
//!
//! `tick` is synchronous and driven by a dedicated daemon thread (see
//! `run_loop`). The adapters are now **non-blocking after Accepted** (AUDIT2
//! B2): the scheduler starts a turn, then drains events across ticks via
...
//!
//! Per tick, in order:
//! 1. **Cancels** (AUDIT2 M2): interrupt + kill the engine of each cancelled
...
//!
//! Job terminal outcome (AUDIT2 M1): an **accepted** turn that fails or times
...
pub struct Scheduler {
    ...
    /// Keys file forwarded to engine adapters for Z.AI profiles (from
    /// `CrewdConfig::keys_env_path`); `None` → `zai-*` profiles fail
...
impl Scheduler {
    ...
    /// engine adapters for Z.AI profiles. Without it, `zai-a`/`zai-p` cells
    /// fail `E_ENGINE_DOWN` at spawn.
    pub fn with_keys_env_path(mut self, path: Option<String>) -> Self {
        self.keys_env_path = path;
        self
    }

    /// Test observability (AUDIT2 M2): whether the cell currently has a live
    ...
    /// Wire the cancel control channel (AUDIT2 M2): the handler sends the cell
    /// name of a cancelled thread and the scheduler interrupts + kills the
    /// engine on the next tick.
    pub fn with_cancel_channel(
        mut self,
    ...
    fn check_death_and_timeout(&mut self, thread_id: &str, now_nanos: u64) -> Result<(), BusError> {
        ...
        if self.supervisor.proc_state(&cell) == Some(crate::engines::EngineProcState::Down) {
            self.fail_turn(thread_id, &cell, &job_id, accepted, "engine process died", "engine_death")?;
            self.in_flight.remove(thread_id);
    ...
    fn handle_timeout(&mut self, thread_id: &str) -> Result<(), BusError> {
        ...
        // PERSIST engine_turn_id + outcome BEFORE interrupt (SPEC §20.9).
        {
            ...
            // Free the cell (AUDIT2 M1): accepted → finished; else requeue.
            if turn.accepted {
    ...
    fn try_start_turn(&mut self, def: &CellDef, now_nanos: u64) -> Result<(), BusError> {
        ...
        match self.supervisor.ensure(&def.name, engine, &cfg) {
            Ok(_) => {}
            Err(e) => {
                self.fail_turn(&thread_id, &def.name, &job_id, false, &e.to_string(), "engine_spawn")?;
                return Ok(());
            }
        }
        ...
        if let Err(e) = self.supervisor.start_turn(&def.name, &payload) {
            self.fail_turn(&thread_id, &def.name, &job_id, false, &e.to_string(), "start_turn")?;
            return Ok(());
        }
    ...
    /// Typed honest resume (AUDIT2 M3): codex reattaches by `engine_thread_id`,
    ...
    /// `finished` (never requeued, cell freed; AUDIT2 M1); non-accepted →
...
/// Boot recovery (AUDIT2 B1 / chaos §20 test b): after a crash, threads left in
...
fn audit_append(
    ...
    let mut chain = audit.lock().map_err(|e| BusError::Internal(e.to_string()))?;
    chain
...
mod tests {
    ...
    fn fixture() -> Scheduler {
        ...
        std::mem::forget(dir); // il file dev'esistere per tutta la durata del test
    ...
    fn spawn_cfg_carries_keys_env_path_from_builder() {
        ...
        let cfg = sched.spawn_cfg(&zai_def());
        assert_eq!(
            cfg.keys_env_path.as_deref(),
            Some("/etc/crewd/keys.env"),
            "lo scheduler deve inoltrare keys_env_path nell'EngineSpawnCfg"
        );
        assert_eq!(cfg.profile.as_deref(), Some("zai-a"));
    }
