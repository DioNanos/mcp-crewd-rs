...
async fn tick(state: &DaemonState) -> Result<(), ()> {
    ...
    for id in &expired_asks {
        state.audit_best_effort("ask_expired", None, None, None, "expired", Some(json!({"ask_id": id})));
...
fn apply_outcome(
    ...
    match outcome {
        Delivered | Acked => {
            ...
            drop(st);
            state.audit_best_effort("delivered", Some(message_id), None, Some(recipient), "ok", None);
        }
        PermanentFailure => {
