...
async fn build_and_start_full(
    ...
    // Boot recovery: resolve orphaned running/spawning threads.
...
/// Spawn the daemon WITH the engine scheduler loop wired. Used by
...
impl CellConn {
    ...
    pub async fn call(
        ...
        bytes.push(b'\n');
        self.stream.write_all(&bytes).await.map_err(|e| WireError {
            code: "E_INTERNAL".into(),
            message: format!("{e}"),
        })?;
...
pub async fn connect_as(
    ...
    let mut stream = UnixStream::connect(&sock).await.map_err(|e| WireError {
        code: "E_INTERNAL".into(),
        message: format!("{e}"),
    })?;
    ...
    bytes.push(b'\n');
    stream.write_all(&bytes).await.map_err(|e| WireError {
        code: "E_INTERNAL".into(),
        message: format!("{e}"),
    })?;
    let resp = read_response(&mut stream).await.map_err(|e| WireError {
        code: "E_INTERNAL".into(),
        message: format!("{e}"),
    })?;
    if let Some(err) = resp.error {
