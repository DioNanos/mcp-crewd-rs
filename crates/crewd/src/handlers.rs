...
use crewd_core::types::{
    clamp_ttl, new_uuidv7, now_rfc3339, rfc3339_after, Envelope, Kind, MsgType, SPEC_VERSION,
    TAINT,
};
use crewd_core::wire::{
    AskParams, AwaitParams, BroadcastParams, CellCancelParams, CellResultParams, CellSendTaskParams,
    CellSpawnParams, CellStatusParams, InboxParams, ReplyParams, SendParams, WireRequest,
    WireResponse,
};
...
pub struct DaemonState {
    /// Shared with the scheduler thread (AUDIT2 B1): one `Store` handle behind
    ...
    pub cfg: CrewdConfig,
    /// Cancel control channel to the scheduler (AUDIT2 M2): the cell name of a
...
impl DaemonState {
    ...
    pub fn audit(
        ...
        chain
            .append(AuditEventDraft::new(kind, message_id, from, to, outcome, detail))
            .map(|_| ())
...
fn build_envelope_with_caps(
    ...
    let mut env = build_envelope(
        kind, msg_type, from, to, body, file_refs, ask_id, reply_to, idempotency_key, ttl_secs,
        seq,
    );
...
pub async fn dispatch(
    ...
    let res = match req.method.as_str() {
        ...
        "report_tool_correlation" => handle_report_tool_correlation(&state, &from, req.params).await,
    ...
    let message_id = {
        ...
        if let Some(hit) = st.dedupe_lookup(from, &key)? {
            if hit.body_sha256 == body_h {
                ...
                return Err(BusError::Dup("idempotency key reused with different body".into()));
            }
        }
        ...
        state.audit("enqueued", Some(&mid), Some(from), Some(&p.to_cell), "ok", None)?;
...
async fn handle_ask(state: &DaemonState, from: &str, params: Value) -> Result<Value, BusError> {
    ...
    let open = state.store.lock().expect("store").pending_asks_count(from)?;
    if let Err(e) = state.quota.lock().expect("quota").check_pending_ask_cap(open) {
        state.audit("quota_exceeded", None, Some(from), Some(&p.to_cell), "rejected", None)?;
        return Err(e);
    }
    let key = p.idempotency_key.unwrap_or_else(new_uuidv7);
    let body_h = body_sha256(&p.body);
    ...
    let message_id = {
        ...
        if let Some(hit) = st.dedupe_lookup(from, &key)? {
            if hit.body_sha256 == body_h {
                return Ok(json!({"ask_id": hit.ask_id, "message_id": hit.message_id, "status": "pending"}));
            } else {
                return Err(BusError::Dup("idempotency key reused with different body".into()));
            }
        }
        ...
        state.audit("enqueued", Some(&mid), Some(from), Some(&p.to_cell), "ok", None)?;
        state.audit("ask_opened", Some(&mid), Some(from), Some(&p.to_cell), "ok", Some(json!({"ask_id": ask_id})))?;
        st.insert_delivery(&mid, &p.to_cell)?;
...
async fn handle_await(
        ...
        .try_activate_await(from, &ask.to_cell)
    {
        Ok(g) => g,
        Err(e) => {
            state.audit("deadlock_prevented", None, Some(from), Some(&ask.to_cell), "rejected", None)?;
            return Err(e);
        }
    };
...
fn answered(state: &DaemonState, ask: &crewd_core::store::AskRow) -> Value {
    let full = ask
        .reply_message_id
        .as_deref()
        .and_then(|mid| state.store.lock().expect("store").get_envelope(mid).ok().flatten());
    match full {
        Some(env) => json!({
            "status": "answered",
    ...
    let outcome_message_id;
    {
        ...
        match st.reply_precheck(&p.ask_id, &env)? {
            ReplyOutcome::Recorded => {
                ...
                state.audit("enqueued", Some(&mid), Some(from), Some(&ask.from_cell), "ok", None)?;
            ...
            ReplyOutcome::Conflict => {
                drop(st);
                state.audit("duplicate_reply", None, Some(from), Some(&ask.from_cell), "dropped", None)?;
                return Err(BusError::ReplyExists(format!("differing reply for {}", p.ask_id)));
            }
        }
    }
...
async fn handle_inbox(state: &DaemonState, from: &str, params: Value) -> Result<Value, BusError> {
    ...
    let arr: Vec<Value> = msgs.iter().filter_map(|e| serde_json::to_value(e).ok()).collect();
...
/// `cell_spawn`: resolve target (named/ephemeral) + idempotent spawn. La
/// primitiva `spawn_idempotent` audita `cell_spawn_requested` (fsync) PRIMA di
/// qualsiasi record consumabile (SPEC §20.10 audit-before-mutation, già R1).
async fn handle_spawn(state: &DaemonState, from: &str, params: Value) -> Result<Value, BusError> {
    ...
    // AUDIT2 B3: clients MUST NOT supply the reserved `~ephemeral-` (or any
    ...
    let req_engine = match p.engine.as_deref() {
        None => None,
        Some(s) => Some(EngineKind::from_db_str(s)
    ...
    let (cell_name, engine, model, profile, cwd) = match target {
        ...
        SpawnTarget::Ephemeral { engine, model, profile, cwd } => {
...
/// `cell_send_task`: follow-up job su un thread (anche terminale). Su thread
/// terminale + engine senza session resume (pi v0) → E_THREAD_NOT_RESUMABLE.
async fn handle_send_task(
    ...
    let terminal = matches!(
        thread.state,
        ThreadState::Done | ThreadState::Interrupted | ThreadState::Timeout | ThreadState::FailedUnknown
    );
...
fn check_thread_owner(state: &DaemonState, from: &str, thread: &CellThread) -> Result<(), BusError> {
...
/// `cell_status`: stato di un thread (owned/admin). Senza `crewd_thread_id` →
/// lista vuota in v0 (richiede enumerazione per principal, non in scope).
async fn handle_status(
    ...
    match &p.crewd_thread_id {
        ...
        None => {
            // v0: enumerazione thread-per-principal differita; ritorna la cella
            // attiva del chiamante se presente.
            let mut out = Vec::new();
...
/// `cell_result`: risultato strutturato di un thread (SPEC §20.10), con i campi
/// ID separati (crewd_thread_id ≠ engine_thread_id ≠ engine_session_id).
async fn handle_result(
    ...
    let result = CellResult {
        final_answer: journal_final_answer(&tail), // smoke-T16 fix: SPEC §20.10, il tail NON è il risultato
        event_tail: tail,
        artifact_refs: vec![],
...
/// (AUDIT2 M2). Ownership-gated like status/result: the cancel is a spawn-class
...
async fn handle_cancel(
    ...
    let (cancelled_jobs, cell_name);
    {
        ...
        let mut n = 0u32;
        for j in &jobs {
            if matches!(j.state, JobState::Queued | JobState::Leased | JobState::Started) {
    ...
    // AUDIT2 M2: signal the scheduler to interrupt + kill the engine process.
...
async fn handle_broadcast(
    ...
    let message_id = {
        ...
        if let Some(hit) = st.dedupe_lookup(from, &key)? {
            if hit.body_sha256 == body_h {
                return Ok(json!({"message_id": hit.message_id, "recipients": included, "status": "enqueued"}));
            } else {
                return Err(BusError::Dup("idempotency key reused with different body".into()));
            }
        }
        ...
        for d in &denied {
            state.audit("protected_access_denied", Some(&mid), Some(from), Some(d), "denied", None)?;
        }
        state.audit(
...
fn authorize_outbound(
    ...
    if let Err(e) = acl.check_reach(from, to, via) {
        state.audit("protected_access_denied", None, Some(from), Some(to), "denied", None)?;
        return Err(e);
    }
    Ok(())
}
...
fn admit_outbound(
    ...
    if let Err(e) = state.quota.lock().expect("quota").check_queue_depth(pending) {
        state.audit("quota_exceeded", None, Some(from), Some(to), "rejected", None)?;
        return Err(e);
    }
    let now = now_ms();
    let sender_res = state
...
/// smoke-T16 fix (SPEC §20.10): `final_answer` è il payload dell'ultimo evento
/// `final:` nel journal (righe "NNNN final: <testo>"), non il tail grezzo.
/// SPEC §20.1: `~ephemeral-<uuid8>`. Il suffisso viene dagli ULTIMI 8 hex
/// dello UUIDv7 (bit random), NON dai primi 8: quelli sono i bit alti del
/// timestamp ms e restano identici per ~65s, facendo collidere due spawn
/// ravvicinati sullo stesso nome cella (`E_CELL_BUSY`, regressione 2026-07-05).
...
mod ephemeral_name_tests {
    ...
    /// Regressione 2026-07-05: i primi 8 hex di un UUIDv7 sono i bit ALTI del
    /// timestamp (cambiano ogni ~65s), quindi due spawn effimeri ravvicinati
    /// generavano lo STESSO nome cella → `E_CELL_BUSY` sul secondo (fan-out
    /// parallelo impossibile). Il suffisso deve venire dai bit random.
    #[test]
    fn two_back_to_back_ephemeral_names_are_distinct() {
        let a = ephemeral_cell_name();
        let b = ephemeral_cell_name();
        assert_ne!(a, b, "nomi effimeri nella stessa finestra timestamp devono differire");
        for n in [&a, &b] {
            assert!(n.starts_with("~ephemeral-"), "prefisso atteso: {n}");
            let suffix = n.strip_prefix("~ephemeral-").unwrap();
            assert_eq!(suffix.len(), 8, "suffisso uuid8: {n}");
            assert!(
                crewd_core::validators::validate_cell_name_allowing_ephemeral(n).is_ok(),
                "deve passare il validator: {n}"
            );
        }
    }
}
...
mod fase2_result_tests {
    ...
    fn final_answer_from_journal_tail() {
        ...
        assert_eq!(journal_final_answer(&tail).as_deref(), Some("risultato vero"));
        assert_eq!(journal_final_answer(&["0001 accepted: t-1".to_string()]), None);
