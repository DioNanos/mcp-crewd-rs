...
async fn handle_connection(
    ...
    // 2. Bounded handshake frame. The FrameReader lives for the whole
    // connection so pipelined bytes past the first `\n` are not lost.
    let mut frames = FrameReader::new();
    let raw = match frames
        .read_bounded_line(&mut stream, MAX_HANDSHAKE_BYTES)
        .await
    {
        Ok(b) => b,
        Err(_) => return,
    ...
    let req: WireRequest = match serde_json::from_slice(&raw) {
        ...
        Err(_) => {
            let _ = send_err(
                &mut stream,
                0,
                BusError::Internal("bad handshake frame".into()),
            )
            .await;
            return;
        }
    ...
    if let Some(tid) = principal.token_id.as_ref() {
        if state.registry.is_revoked(tid) {
            ...
            let _ = send_err(
                &mut stream,
                req.id,
                BusError::AuthRejected("revoked".into()),
            )
            .await;
            return;
        }
...
// macOS has no SO_PEERCRED: uid/gid come from getpeereid(2) (same handshake
// guarantee), the pid is best-effort via LOCAL_PEERPID — consistent with the
// Option<pid> contract of PeerCred.
#[cfg(target_os = "macos")]
fn peercred(stream: &UnixStream) -> io::Result<PeerCred> {
    use std::os::unix::io::AsRawFd;
...
impl FrameReader {
    ...
    async fn read_bounded_line(
        ...
        loop {
            ...
            if n == 0 {
                // EOF without newline: return the residue (pre-fix semantics).
