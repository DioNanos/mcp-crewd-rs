//! Regression tests for the AUDIT3 findings (Fase 1 gate, GPT-5.5).
...
async fn g3_01_audit_write_failure_yields_no_ok() {
    ...
    a.call("cell_send", json!({"to_cell": "codex-audit", "body": "one"}))
        .await
        .unwrap();
    ...
    let err = a
        .call("cell_send", json!({"to_cell": "codex-audit", "body": "two"}))
        .await
        .unwrap_err();
    assert_eq!(err.code, "E_INTERNAL", "no OK when the durable audit write fails");
...
async fn g3_04_op_rpcs_reject_identity_params() {
    ...
    let err = c
        .call("op_audit_verify", json!({"principal_capabilities": ["admin_registry"]}))
        .await
        .unwrap_err();
    assert_eq!(err.code, "E_INTERNAL");
...
async fn g3_02_wrong_token_is_audited() {
    ...
    let err = connect_as(&rt, "dev-senior", "WRONG_TOKEN").await.unwrap_err();
    ...
    assert!(
        evs.iter().any(|e| e["kind"] == "auth_rejected" && e["from"] == "dev-senior"),
...
async fn g3_03_protected_deny_precedes_body_too_large() {
    ...
    let err = b
        ...
        .await
        .unwrap_err();
    assert_eq!(err.code, "E_ACL_DENIED", "protected deny precedes size check");
    assert!(
        audit_events(&rt).iter().any(|e| e["kind"] == "protected_access_denied"),
        "denial audited"
    );
    h.shutdown().await;
}
...
async fn g3_03_protected_deny_precedes_queue_full() {
    ...
    let mut a = connect_as(&rt, "dev-senior", &h.issued_tokens["dev-senior"]).await.unwrap();
    for i in 0..3 {
        a.call("cell_send", json!({"to_cell": "coordinator", "body": format!("m{i}")}))
            .await
            .unwrap();
    }
    ...
    let mut b = connect_as(&rt, "codex-audit", &h.issued_tokens["codex-audit"]).await.unwrap();
    let err = b
        .call("cell_send", json!({"to_cell": "coordinator", "body": "probe"}))
        .await
        .unwrap_err();
    assert_eq!(err.code, "E_ACL_DENIED", "protected deny precedes queue-depth check");
    h.shutdown().await;
}
...
async fn g3_05_edge_released_after_await_ends() {
    ...
    let r1 = a.call("cell_ask", json!({"to_cell": "codex-audit", "body": "a->b"})).await.unwrap();
    let ask_ab = r1["ask_id"].as_str().unwrap().to_string();
    let r2 = b.call("cell_ask", json!({"to_cell": "dev-senior", "body": "b->a"})).await.unwrap();
    ...
    let aw = a.call("cell_await", json!({"ask_id": ask_ab, "timeout_ms": 250})).await.unwrap();
    assert_eq!(aw["status"], "pending");

    // With the edge released, b can now await b->a without E_WOULD_DEADLOCK.
    let ok = b.call("cell_await", json!({"ask_id": ask_ba, "timeout_ms": 100})).await.unwrap();
    assert_eq!(ok["status"], "pending", "edge from the prior await was released");
    h.shutdown().await;
}
...
async fn g3_06_runtime_files_are_0600() {
    ...
    let h = spawn_daemon(&rt).await;
    let mut a = connect_as(&rt, "dev-senior", &h.issued_tokens["dev-senior"]).await.unwrap();
    a.call("cell_send", json!({"to_cell": "codex-audit", "body": "x"})).await.unwrap();

    assert_eq!(mode_of(&rt.join("crewd.db")), 0o600, "DB 0600");
    assert_eq!(mode_of(&rt.join("audit.jsonl")), 0o600, "audit store 0600");
    assert_eq!(mode_of(&rt.join("tokens/dev-senior.token")), 0o600, "token 0600");
...
async fn g3_08_report_before_consume_is_denied() {
    ...
    let r = a
        .call("cell_send", json!({"to_cell": "codex-audit", "body": "do X", "msg_type": "task"}))
        .await
        .unwrap();
    ...
    let err = b
        ...
        .await
        .unwrap_err();
    assert_eq!(err.code, "E_ACL_DENIED", "cannot forge causality before consuming");
