...
fn claude_roundtrip_accepted_final_and_resume_reflects_session() {
    let cfg = claude_cfg(&fixture("fake-claude-shim.mjs"), Some("max"), None, None);
...
fn claude_env_allowlist_no_token_leak() {
    ...
    let cfg = claude_cfg(
        ...
        Some(ProfileDef {
            ...
            api_key_env: "ZAI_API_KEY_A".into(),
        }),
    );
    let mut a = ClaudeAdapter::new(&cfg).unwrap();
...
fn claude_zai_profile_without_keys_is_engine_down() {
    let cfg = claude_cfg(&fixture("env-dump.mjs"), Some("zai-a"), None, None);
