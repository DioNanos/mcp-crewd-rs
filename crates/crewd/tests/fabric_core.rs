//! Fabric core integration tests (crewd Fase 2). Sezione SUPERVISOR (Task 8):
//! - `ensure` due volte stessa cella -> stesso adapter (1 processo per cella)
//! - FakeEngine turno completo produce Accepted -> Final nell'ordine
//! - `stop` + `ensure` -> adapter nuovo
//! - `respawn_backoff_secs` cresce e cappa a 60
//!
//! La "stessa identita' di adapter" si verifica in modo comportamentale via il
//! turn-id del FakeEngine: un adapter riusato prosegue il suo contatore turn
//! (`fake-turn-1`, poi `fake-turn-2`); un adapter nuovo riparte da `fake-turn-1`.
...
fn make_supervisor() -> EngineSupervisor {
    ...
    // Il file viene aperto/chiuso ad ogni append; la directory deve sopravvivere
    // per tutta la durata del test. Leak intenzionale (test breve).
    std::mem::forget(dir);
...
/// Esegue uno start_turn e ritorna l'`engine_turn_id` dell'evento Accepted
/// (poll model AUDIT2 B2: gli eventi arrivano via `poll_events`).
...
fn ensure_twice_same_cell_reuses_adapter() {
    ...
    // Primo ensure + turn -> fake-turn-1 (counter adapter = 1)
    {
        let a = sup.ensure("cell-a", EngineKind::Fake, &cfg).unwrap();
        assert_eq!(accepted_turn_id(&mut **a, "x"), "fake-turn-1");
    }
    // Secondo ensure: STESSO adapter -> il prossimo turn e' fake-turn-2
    // (un adapter nuovo avrebbe ripartito il contatore da fake-turn-1).
    {
        let a = sup.ensure("cell-a", EngineKind::Fake, &cfg).unwrap();
...
fn stop_then_ensure_creates_new_adapter() {
    ...
    // Dopo stop, ensure ricrea l'adapter: il contatore riparte da fake-turn-1.
    {
        let a = sup.ensure("cell-c", EngineKind::Fake, &cfg).unwrap();
        assert_eq!(
...
// ---------------------------------------------------------------------------
// Sezione SCHEDULER (Task 9): happy path + audit order, fail (job accepted
// NON requeued), timeout (FakeEngine hang + timeout_secs=0).
// ---------------------------------------------------------------------------
...
fn scheduler_fixture() -> (
    ...
    std::mem::forget(dir); // il file vive per tutta la durata del test
...
fn scheduler_happy_path_completes_turn_and_audit_order() {
    ...
    // journal registra il final
    let tail = s.journal_tail(&thread_id, 10).unwrap();
    assert!(tail.iter().any(|l| l.contains("final:")), "journal must record final: {tail:?}");

    // audit order: cell_turn_started PRIMA di cell_turn_completed
    ...
    assert!(started.is_some() && completed.is_some(), "audit events missing: {raw}");
    assert!(
...
fn scheduler_fail_path_job_accepted_is_not_requeued() {
    ...
    // FakeEngine che emette Accepted poi Failed.
    ...
    let jobs = s.jobs_for_thread(&thread_id).unwrap();
    assert_eq!(jobs.len(), 1);
    // accepted (prima del Failed) e NON requeued
    assert!(
...
// Regressione retry-storm (smoke 2026-07-05): una spawn-failure PRE-accettazione
// (`engine_spawn`, es. profilo zai-a senza keys_env_path → E_ENGINE_DOWN) non
// deve essere ri-accodata. Il vecchio path `job_requeue` la rimetteva 'queued'
// ad ogni tick → hot-loop che gonfiava audit.jsonl/WAL. Ora è terminale.
#[test]
fn engine_spawn_failure_is_terminal_not_requeued() {
    ...
    let thread_id = {
        let s = store.lock().unwrap();
        let mut ch = audit.lock().unwrap();
        // Cella claude con profilo zai-a ma nessun keys_env_path cablato:
        // make_adapter → ClaudeAdapter::new → build_env fallisce E_ENGINE_DOWN.
        s.cell_register(
            &CellDef {
                name: "zai".into(),
    ...
    // tick 1: lo spawn dell'engine fallisce prima dell'accettazione.
    sched.tick().unwrap();
    {
        let s = store.lock().unwrap();
        let t = s.thread_get(&thread_id).unwrap().unwrap();
        assert_eq!(t.state, ThreadState::FailedUnknown, "thread deve fallire");
        let jobs = s.jobs_for_thread(&thread_id).unwrap();
        assert_eq!(jobs.len(), 1);
        assert_ne!(
            jobs[0].state,
            JobState::Queued,
            "una spawn-failure non-accettata NON deve tornare 'queued' (evita hot-loop)"
        );
    }

    // tick 2: nessun nuovo tentativo — un solo cell_turn_started nell'audit.
    sched.tick().unwrap();
    let raw = std::fs::read_to_string(&audit_path).unwrap();
    let started = raw.matches("\"kind\":\"cell_turn_started\"").count();
    assert_eq!(
        started, 1,
        "una sola spawn-attempt, non un retry-storm; cell_turn_started trovati: {started}"
    );
}
...
fn scheduler_timeout_hang_persists_engine_turn_id_before_interrupt() {
    ...
    // tick 1: avvia il turno (hang: solo Accepted) -> thread running, in_flight
    sched.tick().unwrap();
    {
        let s = store.lock().unwrap();
        let t = s.thread_get(&thread_id).unwrap().unwrap();
    ...
    // tick 2: timeout (delta > 0 con timeout_secs=0) -> thread timeout
    sched.tick().unwrap();
    let s = store.lock().unwrap();
    let t = s.thread_get(&thread_id).unwrap().unwrap();
    ...
    let raw = std::fs::read_to_string(&audit_path).unwrap();
    assert!(raw.contains("\"kind\":\"cell_timeout\""), "missing cell_timeout audit: {raw}");
}

#[test]
fn scheduler_cancel_interrupts_and_stops_engine() {
    // AUDIT2 M2: a cancel signalled on the control channel makes the scheduler
    ...
    sched.tick().unwrap();
    assert!(sched.engine_active("cx"), "engine live after the hang turn starts");
    ...
    assert!(
        ...
        "cancel must interrupt + stop the cell engine (AUDIT2 M2)"
    );
}

// ---------------------------------------------------------------------------
// Sezione CLAUDE (Task 12-rust): ClaudeAdapter via node shim. Usa bin_override
// su tests/fixtures/* cosicche' i test girano senza Agent SDK / rete.
// ---------------------------------------------------------------------------
...
/// Poll a child adapter (poll model AUDIT2 B2) until a terminal `Final`/`Failed`
...
fn claude_roundtrip_accepted_final_and_resume_reflects_session() {
    ...
    // resume: il prossimo turn riflette il session id passato al shim (typed
    // resume_session, AUDIT2 M3).
...
fn claude_env_allowlist_no_token_leak() {
    ...
    .unwrap();
    std::mem::forget(keys_dir); // il file keys sopravvive per la durata del test
    ...
    // 1. ogni nome di env var visto dal shim MUST essere nella allowlist (§20.7)
    for name in &names {
        assert!(
    ...
    // 2. AUDIT2 N1: la key GREZZA `ZAI_API_KEY_A` NON deve passare al figlio (solo
    //    la sua mappatura verso l'allowlisted `ANTHROPIC_AUTH_TOKEN`), e la
    //    mappatura DEVE essere avvenuta — non piu' tautologico.
    assert!(
    ...
    // 3. il valore del token MUST NON apparire mai come NOME di env var
    assert!(
...
fn claude_abort_emits_error_and_shutdown_terminates() {
    ...
    a.interrupt().unwrap(); // abort → shim emette error "aborted"
...
// ---------------------------------------------------------------------------
// Sezione HANDLERS (Task 10): spawn→status→result; ACL spawn; status altrui;
// audit cell_spawn_requested PRIMA del thread record (hash-chain).
// ---------------------------------------------------------------------------
...
async fn handler_spawn_status_result_roundtrip() {
    ...
    // Campi ID separati (SPEC §20.2): sempre presenti, anche se None pre-scheduling.
    ...
    assert!(
        ["done", "interrupted", "timeout", "failed_unknown", "running", "spawning"]
            .contains(&exit),
...
async fn handler_spawn_denied_without_spawn_capability() {
    let dir = tempdir().unwrap();
    let handle = crewd::testkit::spawn_daemon(dir.path()).await;
    // vault non ha capability "spawn" nell'ACL di test.
...
async fn handler_status_other_thread_denied_without_admin() {
    ...
    let handle = crewd::testkit::spawn_daemon(dir.path()).await;
    register_named_cell(&handle.state, "worker-a", EngineKind::Fake);
    // dev-senior (con spawn) crea il thread → created_by_principal = dev-senior.
    ...
    // codex-audit (ha spawn, NON admin_registry) → status altrui = E_ACL_DENIED.
...
async fn handler_spawn_audit_cell_spawn_requested_present_and_chain_verifies() {
    ...
    // L'evento cell_spawn_requested è audited (fsync) PRIMA del record thread,
    // e la chain hash-linked verifica end-to-end.
...
// ---------------------------------------------------------------------------
// Sezione PI (Task 13): PiAdapter via trace replay (no session resume).
// ---------------------------------------------------------------------------
...
fn pi_resume_is_thread_not_resumable() {
    // Both typed resume paths are refused honestly (SPEC §20.6 / AUDIT2 M3).
    let mut a = PiAdapter::new(&pi_replay_cfg()).unwrap();
    assert_eq!(a.resume_session("any-sid").unwrap_err().code(), "E_THREAD_NOT_RESUMABLE");
    assert_eq!(a.resume_thread("any-tid").unwrap_err().code(), "E_THREAD_NOT_RESUMABLE");
    a.shutdown();
}

/// Regressione smoke-T16: una cella EFFIMERA (`~ephemeral-*`, non nel registry)
/// con job queued DEVE essere scoperta e schedulata dal tick — prima del fix il
/// tick iterava solo `cell_list_defs()` e i thread effimeri restavano
/// `spawning` per sempre.
#[test]
fn scheduler_starts_ephemeral_cells_not_in_registry() {
    ...
    let thread_id = {
        let s = store.lock().unwrap();
        let mut ch = audit.lock().unwrap();
        // NIENTE register_cell: il target è effimero, via spawn_idempotent reale.
...
/// Regressione smoke-T16: il child engine DEVE girare nella cwd richiesta
/// (senza `current_dir` ereditava la cwd del daemon e la cella lavorava sul
