//! Chaos matrix + crash consistency (crewd Phase 2 Task 15). One test per row:
//!
...
fn chaos_a_kill9_engine_during_accepted_turn() {
    ...
    assert!(
        failed,
        "thread must become failed_unknown after engine kill"
    );

    {
        let s = store.lock().unwrap();
        let jobs = s.jobs_for_thread(&tid).unwrap();
        // accepted job NOT requeued (SPEC §20.5): finished, cell freed.
        assert_ne!(
            jobs[0].state,
            JobState::Queued,
            "accepted job must never be requeued"
        );
        ...
        assert!(
            tail.iter().any(|l| l.contains("accepted:")),
            "journal tail: {tail:?}"
        );
    }
    ...
    assert!(
        raw.contains("\"kind\":\"cell_turn_failed\""),
        "missing cell_turn_failed: {raw}"
    );
    assert!(matches!(
    ...
    assert!(
        recovered,
        "explicit follow-up must run on a respawned engine"
    );
    {
        let s = store.lock().unwrap();
        let jobs = s.jobs_for_thread(&tid).unwrap();
...
async fn chaos_b_kill9_daemon_during_turn() {
    ...
    // spawn_requests intact: the same key replays to the SAME thread.
    {
        ...
        let r = c
            ...
            .unwrap();
        assert_eq!(
            r["crewd_thread_id"].as_str().unwrap(),
            tid,
            "replay same thread"
        );
...
async fn chaos_c_double_primary_idempotency_and_busy() {
    ...
    let r2 = c2
        ...
        .unwrap();
    assert_eq!(
        r2["crewd_thread_id"].as_str().unwrap(),
        tid,
        "same thread on replay"
    );
...
async fn chaos_d_disconnect_turn_completes_result_from_new_conn() {
    ...
    while start.elapsed() < Duration::from_secs(10) {
        ...
        if r["exit_status"] == serde_json::json!("done") {
            ...
            tail_has_final = r["event_tail"]
                ...
                .map(|a| {
                    a.iter()
                        .any(|l| l.as_str().unwrap_or("").contains("final:"))
                })
                .unwrap_or(false);
            break;
        }
    ...
    assert!(
        ...
        "background turn must complete after client disconnect"
    );
    assert!(
        tail_has_final,
        "result event_tail must contain the final answer"
    );
    h.shutdown().await;
}
...
async fn chaos_e_audit_chain_verifies_through_crash() {
    ...
    // hash-chain is intact across the crash + boot recovery.
    {
        let mut c = connect_as(&rt, "primary", &token).await.unwrap();
        let v = c
            .call("op_audit_verify", serde_json::json!({}))
            .await
            .unwrap();
