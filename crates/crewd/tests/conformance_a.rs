...
async fn envelope_authority_rejects_client_identity() {
    ...
    assert_eq!(
        mid.split('-').map(|s| s.len()).collect::<Vec<_>>(),
        vec![8, 4, 4, 4, 12]
    );
    ...
    let caps: Vec<String> = msgs[0]["principal_capabilities"]
        ...
        .collect();
    assert!(
        caps.contains(&"send".to_string()),
        "sender caps asserted: {caps:?}"
    );
    h.shutdown().await;
}
...
async fn ask_returns_pending_within_2s_and_reply_roundtrip() {
    ...
    let r = a
        .call(
            "cell_ask",
            json!({"to_cell": "codex-audit", "body": "safe to merge?"}),
        )
        .await
        .unwrap();
    assert!(
        t0.elapsed() < Duration::from_secs(2),
        "cell_ask MUST return within 2s"
    );
    ...
    let rr = b
        .call(
            "cell_reply",
            json!({"ask_id": ask_id, "body": "yes, after CI"}),
        )
        .await
        .unwrap();
    ...
    assert_eq!(
        aw["reply"]["reply_to"].as_str().unwrap(),
        r["message_id"].as_str().unwrap()
    );
    h.shutdown().await;
}
...
async fn second_differing_reply_rejected_e_reply_exists() {
    ...
    let r = a
        ...
        .unwrap();
    let ask_id = r["ask_id"].as_str().unwrap().to_string();
    b.call("cell_reply", json!({"ask_id": ask_id, "body": "first"}))
        .await
        .unwrap();
    ...
    let err = b
        .call(
            "cell_reply",
            json!({"ask_id": ask_id, "body": "second, different"}),
        )
        .await
        .unwrap_err();
...
async fn await_non_owned_ask_immediate_e_ask_not_found() {
    ...
    let err = a
        .call(
            "cell_await",
            json!({"ask_id": "01980000-0000-7000-8000-000000000000"}),
        )
        .await
        .unwrap_err();
    assert_eq!(err.code, "E_ASK_NOT_FOUND");
...
async fn deadlock_fires_only_at_await_not_at_ask() {
    ...
    assert!(
        has_event(&rt, "deadlock_prevented"),
        "deadlock_prevented audited"
    );

    let waited = waiter.await.unwrap().unwrap();
    assert_eq!(
        waited["status"], "pending",
        "the awaiting side just times out"
    );
...
async fn protected_send_ask_gated_by_per_target_grant() {
    ...
    let r = c
        .call(
            "cell_ask",
            json!({"to_cell": "codex-audit", "body": "status?"}),
        )
        .await
        .unwrap();
    let ask_id = r["ask_id"].as_str().unwrap().to_string();
...
async fn broadcast_omits_protected_without_grant_and_audits() {
    ...
    let mut c = connect_as(&rt, "coordinator", &h.issued_tokens["coordinator"])
        ...
        .unwrap();
    let r = c
        .call("cell_broadcast", json!({"body": "maintenance 18:00"}))
        .await
        .unwrap();
    ...
    let mut a = connect_as(&rt, "dev-senior", &h.issued_tokens["dev-senior"])
        ...
        .unwrap();
    let r = a
        .call("cell_broadcast", json!({"body": "ping"}))
        .await
        .unwrap();
    let recipients: Vec<String> = r["recipients"]
        .as_array()
    ...
    assert!(
        recipients.contains(&"vault".to_string()),
        "grant includes vault: {recipients:?}"
    );
...
async fn quota_pending_ask_cap_10() {
    ...
    for i in 0..10 {
        a.call(
            "cell_ask",
            json!({"to_cell": "codex-audit", "body": format!("q{i}")}),
        )
        .await
        .unwrap();
    }
...
async fn queue_overflow_rejects_new_e_quota() {
    ...
    for i in 0..5 {
        a.call(
            "cell_send",
            json!({"to_cell": "codex-audit", "body": format!("m{i}")}),
        )
        .await
        .unwrap();
    }
    let err = a
        .call(
            "cell_send",
            json!({"to_cell": "codex-audit", "body": "overflow"}),
        )
        .await
        .unwrap_err();
...
async fn same_idempotency_key_same_message_id() {
    ...
    let r1 = a
        .call(
            "cell_send",
            json!({"to_cell": "codex-audit", "body": "same", "idempotency_key": key}),
        )
        .await
        .unwrap();
    let r2 = a
...
async fn same_key_different_body_e_dup() {
    ...
    a.call(
        "cell_send",
        json!({"to_cell": "codex-audit", "body": "one", "idempotency_key": key}),
    )
    .await
    .unwrap();
    let err = a
        .call(
            "cell_send",
            json!({"to_cell": "codex-audit", "body": "two", "idempotency_key": key}),
        )
        .await
        .unwrap_err();
