...
pub(crate) async fn connect(
    ...
    let mut bytes = serde_json::to_vec(&req).map_err(|e| wire_err("E_INTERNAL", e.to_string()))?;
    bytes.push(b'\n');
    stream
        .write_all(&bytes)
        .await
...
pub async fn audit_verify(runtime_dir: &Path, cell: &str, token: &str) -> Result<String, String> {
    ...
    match status {
        ...
        "broken" => {
            let event_id = res.get("event_id").and_then(|v| v.as_str()).unwrap_or("");
