...
async fn audit_verify_prints_ok_then_broken_after_flip() {
    ...
    let ok = crew::ops::audit_verify(rt, "dev-senior", &token).await.unwrap();
    ...
    let broken = crew::ops::audit_verify(rt, "dev-senior", &token).await.unwrap();
...
async fn inspect_returns_envelope_and_audit_events() {
    ...
    let res = c
        ...
        .await
        .unwrap();
    let mid = res.get("message_id").and_then(|v| v.as_str()).unwrap().to_string();
    let out = crew::ops::inspect(rt, "dev-senior", &token, &mid).await.unwrap();
