...
//! `crewd::testkit`. The third test is the Phase 1 smoke: two shims do
...
async fn tools_list_exposes_seven_cell_tools_with_hints() {
    ...
    // 7 Phase 1 bus tools + fabric tools (Task 14); assert at least the 7 original.
    assert!(
        tools.len() >= 7,
        "expected at least 7 tools, got {}",
        tools.len()
    );
...
#[tokio::test]
async fn cell_spawn_profile_schema_is_config_driven_not_hardcoded() {
    let dir = tempfile::tempdir().unwrap();
    let h = spawn_daemon(dir.path()).await;
    let token = h.issued_tokens.get("dev-senior").unwrap().clone();
    let (mut w, mut r) = spawn_shim(dir.path(), "dev-senior", &token);
    call(&mut w, &mut r, 1, "initialize", json!({})).await;
    let list = call(&mut w, &mut r, 2, "tools/list", json!({})).await;
    let spawn = list["result"]["tools"]
        .as_array()
        .unwrap()
        .iter()
        .find(|t| t["name"] == "cell_spawn")
        .expect("cell_spawn tool present");
    let profile = &spawn["inputSchema"]["properties"]["profile"];
    // The profile is config-driven: a free string naming a profile declared in
    // crewd.toml (or "max"), NOT a fixed enum that leaks any internal scheme.
    assert_eq!(
        profile["type"], "string",
        "profile must be a string: {profile}"
    );
    assert!(
        profile.get("enum").is_none(),
        "profile must not be a hardcoded enum: {profile}"
    );
    assert!(
        profile["description"].as_str().is_some(),
        "profile must carry a description: {profile}"
    );
    h.shutdown().await;
}
