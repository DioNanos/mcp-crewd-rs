...
**Spec version:** `0.1`
**Status:** Draft (Phase 0) — normative, to be implemented 1:1 in Phase 1.
**Binding inputs:** an external design/audit review
(findings F1, F3, F5, F9 — and F4 for the adapter contract); README charter.

This document is the normative protocol that the Phase 1 `crewd` daemon and the
...
- **D8 — Default-deny semantics clarified (§11).** The security posture review
  ...
  reviewer.
- **D9 — G1 gate fixes applied.** Following the G1 verdict (Phase 0 approved
...
- **D10 — D2 protected cells replace a full per-recipient matrix.** CR-A-04
  ...
  than a complete per-recipient ACL matrix — a coordinator decision (D2).
...
- **D12 — Follow-up review round gate fixes.** Following a follow-up external
  security-posture review (NEEDS_CHANGES, 2 MAJOR): (1) `cell_broadcast` now evaluates
